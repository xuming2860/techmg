        ...
        <el-table-column label="操作" width="200" fixed="right">
          <template #default="{row}">
            ...
            <el-button type="success" link size="small" @click="handleAdd(row)"
              v-if="row.type!==2">添加子项</el-button>
    ...
    <el-dialog v-model="dialog.visible" :title="dialogTitle" width="560px">
      <el-form ref="formRef" :model="dialog.form" :rules="rules" label-width="80px">
        <el-form-item label="上级菜单">
          <el-tree-select v-model="dialog.form.parentId" :data="tableData"
            :props="{label:'menuName',value:'id'}" placeholder="不选则为顶级" check-strictly clearable style="width:100%" />
        </el-form-item>
...
async function fetchData() {
  loading.value = true
  try { tableData.value = await getMenuTree() || [] }
...
const dialog = reactive({
  ...
  form: { id:null, parentId:null, menuName:'', type:1, path:'', component:'', icon:'', perms:'', sort:0, status:1, visible:1 }
})
const dialogTitle = computed(() => dialog.isEdit?'编辑菜单':(dialog.form.parentId?'添加子菜单':'新增顶级菜单'))
const rules = {
...
function handleAdd(parent) {
  dialog.isEdit = false
  Object.assign(dialog.form, { id:null, parentId:parent?.id||null, menuName:'', type:parent?1:0, path:'', component:'', icon:'', perms:'', sort:0, status:1, visible:1 })
  dialog.visible = true
}
...
async function handleSubmit() {
  ...
  try {
    ...
    if (data.type === 0) { data.path = ''; data.component = '' }
...
function handleDelete(row) {
  ...
  ElMessageBox.confirm(hasChildren?`「${row.menuName}」有子菜单，删除后子菜单也将无法访问。确定删除?`:`确定删除「${row.menuName}」?`, '确认', { type:'warning' })
    .then(async () => { await deleteMenu(row.id); ElMessage.success('已删除'); fetchData() })
}
