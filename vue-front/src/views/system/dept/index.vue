      ...
      <el-table :data="tableData" border stripe row-key="id" v-loading="loading"
        ...
        <el-table-column label="操作" width="200" fixed="right">
          <template #default="{row}">
            ...
            <el-button type="success" link size="small" @click="handleAdd(row)">添加子部门</el-button>
      ...
      <el-form ref="formRef" :model="dialog.form" :rules="rules" label-width="80px">
        <el-form-item label="上级部门">
          <el-tree-select v-model="dialog.form.parentId" :data="tableData"
            :props="{label:'deptName',value:'id'}" placeholder="不选则为顶级" check-strictly clearable style="width:100%" />
        </el-form-item>
...
async function fetchData() {
  loading.value = true
  try { tableData.value = await getDeptTree() || [] }
...
const dialogTitle = computed(() => dialog.isEdit?'编辑部门':(dialog.form.parentId?'添加子部门':'新增顶级部门'))
const rules = {
...
function handleAdd(parent) {
  dialog.isEdit = false
  Object.assign(dialog.form, { id:null, parentId:parent?.id||null, deptName:'', deptCode:'', sort:0, status:1 })
  dialog.visible = true
}
...
function handleDelete(row) {
  ...
  ElMessageBox.confirm(hasChildren?`「${row.deptName}」有子部门，确定删除?`:`确定删除「${row.deptName}」?`, '确认', { type:'warning' })
    .then(async () => { await deleteDept(row.id); ElMessage.success('已删除'); fetchData() })
}
