  ...
  <div class="page-container">
    <el-card>
      ...
      <el-form :inline="true" :model="queryForm" class="search-bar">
        <el-form-item label="关键字">
          <el-input v-model="queryForm.keyword" placeholder="认证号/姓名" clearable @keyup.enter="handleSearch" />
        </el-form-item>
        <el-form-item>
      ...
      <el-pagination class="pagination" background layout="total,sizes,prev,pager,next,jumper"
        ...
        :page-sizes="[10,20,50]" :total="total" @change="fetchData" />
    </el-card>
    ...
    <el-dialog v-model="dialog.visible" :title="dialog.isEdit?'编辑用户':'新增用户'" width="520px">
      <el-form ref="formRef" :model="dialog.form" :rules="rules" label-width="80px">
        <el-form-item label="认证号" prop="authNo">
          <el-input v-model="dialog.form.authNo" :disabled="dialog.isEdit" placeholder="统一认证号" />
        </el-form-item>
        ...
        <el-form-item label="密码" v-if="!dialog.isEdit">
          <el-input v-model="dialog.form.password" type="password" placeholder="登录密码" show-password />
        </el-form-item>
        ...
        <el-form-item label="部门">
          <el-tree-select v-model="dialog.form.deptId" :data="deptTree"
            :props="{label:'deptName',value:'id'}" placeholder="选择部门" check-strictly clearable style="width:100%" />
        </el-form-item>
    ...
    <el-dialog v-model="roleDialog.visible" title="分配角色" width="460px">
      ...
      <template #footer>
        ...
        <el-button type="primary" @click="handleRoleSubmit" :loading="roleDialog.loading">保存</el-button>
      </template>
    </el-dialog>
  </div>
</template>
...
import { getUserList, createUser, updateUser, deleteUser, assignUserRoles, getUserRoles } from '@/api/system/user'
...
function handleReset() { queryForm.keyword = ''; queryForm.page = 1; fetchData() }

const formRef = ref(null)
const dialog = reactive({
  visible:false, isEdit:false, loading:false,
  form: { id:null, authNo:'', realName:'', username:'', password:'', email:'', phone:'', deptId:null, status:1 }
})
const rules = {
...
function handleAdd() {
  dialog.isEdit = false
  Object.assign(dialog.form, { id:null, authNo:'', realName:'', username:'', password:'', email:'', phone:'', deptId:null, status:1 })
  dialog.visible = true
}
...
function handleDelete(row) {
  ElMessageBox.confirm(`确定删除「${row.realName||row.authNo}」?`, '确认', { type:'warning' })
    .then(async () => { await deleteUser(row.id); ElMessage.success('已删除'); fetchData() })
}

const roleDialog = reactive({ visible:false, loading:false, userId:null, allRoles:[], checkedRoles:[] })
async function handleRole(row) {
  roleDialog.userId = row.id; roleDialog.visible = true; roleDialog.loading = true
  try {
    const [roles, userRoles] = await Promise.all([getRoleList({ page:1, size:100 }), getUserRoles(row.id)])
...
async function handleRoleSubmit() {
  ...
  try { await assignUserRoles(roleDialog.userId, roleDialog.checkedRoles); ElMessage.success('角色分配成功'); roleDialog.visible = false }
  finally { roleDialog.loading = false }
}

onMounted(async () => {
  fetchData()
  try { deptTree.value = await getDeptTree() } catch(e){/*empty*/}
})
</script>

<style lang="scss" scoped>
.page-container { padding:0; }
.card-header { display:flex; justify-content:space-between; align-items:center; h2 { margin:0; font-size:18px; } }
