<template>
  <div class="page-container">
    ...
    <!-- 编辑弹窗 -->
    <el-dialog v-model="dialog.visible" :title="dialog.isEdit?'编辑角色':'新增角色'" width="480px">
      <el-form ref="formRef" :model="dialog.form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="角色编码" prop="roleCode">
          <el-input v-model="dialog.form.roleCode" :disabled="dialog.isEdit" placeholder="如: PLATFORM_ADMIN" />
        </el-form-item>
    ...
    <el-dialog v-model="menuDialog.visible" title="分配菜单" width="400px">
      <el-tree ref="menuTreeRef" :data="menuDialog.tree" show-checkbox node-key="id"
        :default-checked-keys="menuDialog.checkedIds" :props="{label:'menuName',children:'children'}"
        default-expand-all check-strictly v-loading="menuDialog.loading" />
      <template #footer>
        <el-button @click="menuDialog.visible=false">取消</el-button>
        <el-button type="primary" @click="handleMenuSubmit" :loading="menuDialog.loading">保存</el-button>
      </template>
    </el-dialog>
  </div>
</template>
...
function handleAdd() {
  dialog.isEdit = false
  Object.assign(dialog.form, { id:null, roleName:'', roleCode:'', description:'', sort:0, status:1 })
  dialog.visible = true
}
...
function handleDelete(row) {
  ElMessageBox.confirm(`确定删除「${row.roleName}」?`, '确认', { type:'warning' })
    .then(async () => { await deleteRole(row.id); ElMessage.success('已删除'); fetchData() })
}

const menuTreeRef = ref(null)
const menuDialog = reactive({ visible:false, loading:false, roleId:null, tree:[], checkedIds:[] })
async function handleMenu(row) {
  menuDialog.roleId = row.id; menuDialog.visible = true; menuDialog.loading = true
  try {
  ...
  } finally { menuDialog.loading = false }
}
